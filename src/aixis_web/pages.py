...
@page_router.get("/whitepaper")
async def whitepaper_redirect():
    """Phase E-2: friendly URL for the Aixis 監査方法論書 PDF.

    The PDF itself is a build-time artifact generated by
    ``scripts/build_whitepaper.py`` and served from
    ``/static/pdf/aixis-audit-methodology-v1_0.pdf``. The filename
    is version-suffixed so that long-lived ``Cache-Control: immutable``
    on ``/static/`` never serves a stale copy — bump the suffix in
    both places whenever the methodology version changes.
    """
    return RedirectResponse(
        url="/static/pdf/aixis-audit-methodology-v1_0.pdf",
        status_code=302,
    )
