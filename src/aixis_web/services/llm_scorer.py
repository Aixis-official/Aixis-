...
- 「〜に『〜』と表示されている」「〜が正確に出力されている」等の**検品チェックリスト的な記述**
...
- evidence には**ユーザー体験に基づく具体的な根拠**を書くこと。「概ね良い」「問題ない」のような曖昧な表現は不可。
  ○「日本語で指示すると自然な日本語スライドが返ってくる。社内向け資料ならそのまま使えるレベルである」
  ○「応答に約30秒かかるが、手作業でスライドを作る工数を考えれば十分な速さである」
  ×「表紙に『〇〇』と日本語で正確に表示されている」（← 検品報告。このような書き方は禁止）
  ×「日本語のスライドが生成されることが確認された」（← 検査結果。このような書き方は禁止）
