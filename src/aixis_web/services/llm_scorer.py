...
CATEGORY_PROMPT_CONFIG = {
    "slide_creation": {
        ...
        "axis_differentiation": (
            ...
            "- **信頼性・安全性**: 事実の正確性・ハルシネーション・出典・内部一貫性・データの取り扱い信頼性・"
            "情報の検証可能性・出力の再現性（言語品質・構成力・応答時間には触れない）\n"
    ...
    "translation": {
        "review_target": (
            "英語の文書を日本語に翻訳するAIツールを、実際に使ってみた結果をレビューする。\n"
            "各検証では、英語の原文を入力し、ツールが出力した日本語訳文を確認している。\n"
            "あなたの役割は「このツールで翻訳した結果、実務で使えるか？」を利用者目線で評価することである。\n\n"
            "【重要】各検証には「検証タイプ」が明記されている。検証タイプに応じて評価の重点を変えること:\n"
            "- 契約書翻訳: 法律用語の正確性、条項構造の保持が命\n"
            "- プレスリリース翻訳: 日本のビジネス慣習への適応、敬語レベルが鍵\n"
            "- 技術レポート翻訳: 専門用語の的確な訳出、多義語の文脈判断が重要\n\n"
            "【翻訳品質の判断基準】\n"
            "利用者が最も気にするのは以下の3点である:\n"
            "1. 原文の情報が正確に伝わっているか（誤訳・訳抜けがないか）\n"
            "2. 自然な日本語になっているか（翻訳調・直訳調でないか）\n"
            "3. 原文にない情報が勝手に追加されていないか（ハルシネーション）\n\n"
            "【文体の注意】\n"
            "evidenceやstrengths/risksには、利用者が読んで役立つ具体的な記述をすること。\n"
            "「〜が確認された」「〜と評価できる」のような監査報告書の文体は禁止。\n"
            "「〜できる」「〜になっている」「〜が目立つ」のような、実際に使った感想として自然な文体で書くこと。"
        ),
        ...
        "axis_differentiation": (
            ...
            "- **信頼性・安全性**: ハルシネーション有無・訳抜け・数値正確性・意味の保全・情報の検証可能性・"
            "翻訳結果の信頼性（日本語品質・構造保持・応答時間には触れない）\n"
    ...
    "meeting_minutes": {
        ...
        "axis_differentiation": (
            ...
            "- **信頼性・安全性**: 録音への忠実性・数値正確性・ハルシネーション有無・発言帰属・"
            "情報の改変リスク・出力の信頼性（言語品質・構造化能力・応答時間には触れない）\n"
...
class LLMScorer:
    ...
    # Safety axis also requires Sonnet: factual accuracy, hallucination detection,
    # and evidence-based analysis need deeper reasoning than Haiku provides.
    SONNET_REQUIRED_AXES = frozenset({"localization", "practicality", "safety"})
        ...
        if len(loaded_images) <= BATCH_SIZE:
            ...
            if len(batch_responses) == 1:
                ...
                synthesis_prompt = (
                    ...
                    f"strengths と risks もそれぞれ最低3項目含めてください。各項目は2〜3文の具体的な説明にしてください（1文だけの短い記述は不可）。\n\n"
                )
...
{{
  ...
  "strengths": ["強み1: 2〜3文で具体的に記述", "強み2: ...", "強み3: ..."],
  "risks": ["注意点1: 2〜3文で具体的に記述", "注意点2: ...", "注意点3: ..."]
}}
...
- strengths と risks もそれぞれ最低3項目ずつ含めること。各項目は**2〜3文の具体的な説明**にすること（1文だけの短い記述は不可）。
  ユーザーが読んで「なるほど」と思える根拠・具体例・実務への影響を必ず添えること。
  ○ 良い例:「複数の専門分野（法律、技術、財務）の文書を高い精度で日本語に翻訳できる。法律用語や会計用語など、分野固有の訳語が適切に選択されており、翻訳後の文書をそのまま社内利用できるレベルの品質である。」
  × 悪い例:「標準的なセキュリティ水準」（← 1文で根拠なし。禁止）
- 日本語で記述すること
"""
    ...
    _NEGATIVE_MARKERS = (
        ...
        "最低限の",  # when describing limitations (removed "基本的な"/"標準的な" — too aggressive)
    )
