...
CATEGORY_PROMPT_CONFIG = {
    ...
    "meeting_minutes": {
        ...
        "axis_differentiation": (
            "## 軸間の差別化ルール（重要）\n"
            "各評価軸は固有の観点を持っている。他の軸と同じ内容を繰り返してはならない。\n"
            "- **実務適性**: 文字起こし精度・要約品質・アクションアイテム抽出・話者識別（言語品質やROIには触れない）\n"
            "- **費用対効果**: ROI・処理速度・手直し工数・価格に見合う価値（文字起こし精度や言語品質には触れない）\n"
            "- **日本語能力**: 出力言語・話し言葉→書き言葉変換・敬語・ビジネス表現（構造化能力やROIには触れない）\n"
            "- **信頼性・安全性**: 録音への忠実性・数値正確性・ハルシネーション有無・発言帰属（言語品質や構造化能力には触れない）\n"
            "- **革新性**: 自動要約・議題分割・出力形式・UI・連携機能（基本的な文字起こし能力には触れない）"
        ),
